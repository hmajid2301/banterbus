...
func (q *Queries) GetAllPlayerAnswerIsReady(ctx context.Context, playerID uuid.UUID) ([]GetAllPlayerAnswerIsReadyRow, error) {
	rows, err := q.db.Query(ctx, getAllPlayerAnswerIsReady, playerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
